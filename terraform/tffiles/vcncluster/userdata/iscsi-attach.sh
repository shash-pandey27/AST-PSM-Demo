...
echo "Waiting to execute isci-attach.sh. time is now $(date -R)!" | tee $HOME/iscsi-attach-output.txt
# Added a wait time to ensure all volumes get attached before script is executed
sleep 60
echo "Executing isci-attach.sh. time is now $(date -R)!" | tee $HOME/iscsi-attach-output.txt
